...
--changeset ${vr_owner_user}_CODE:PACKAGE_BODY_PKG_STAGING runOnChange:TRUE  failOnError:TRUE splitStatements:FALSE stripComments:FALSE


  CREATE OR REPLACE PACKAGE BODY "PKG_STAGING" as

function f_get_partition_name (a_source_system SOURCE_SYSTEM.SOURCE_SYSTEM_ID%TYPE,a_asofdate AGE.ASOFDATE%TYPE, a_tablename VARCHAR2) return VARCHAR2
is
...
function f_get_subpartition_name (a_source_system SOURCE_SYSTEM.SOURCE_SYSTEM_ID%TYPE,a_asofdate AGE.ASOFDATE%TYPE, a_tablename VARCHAR2) return VARCHAR2
is
...
function f_get_max_high_value (a_source_system SOURCE_SYSTEM.SOURCE_SYSTEM_ID%TYPE,a_asofdate AGE.ASOFDATE%TYPE, a_tablename VARCHAR2) return date
is
v_maxdate date;
v_number       NUMBER(15);
begin
    ...
    OPEN table_list_cur;
     ...
     LOOP
        ...
        dbms_output.put_line(table_list.table_name);     
         v_partition:=PKG_STAGING.F_GET_PARTITION_NAME (a_source_system,a_asofdate,table_list.table_name);
         v_subpartition:=PKG_STAGING.F_GET_SUBPARTITION_NAME (a_source_system,a_asofdate,table_list.table_name);
         v_maxdate:=PKG_STAGING.F_GET_MAX_HIGH_VALUE (a_source_system,a_asofdate,table_list.table_name);
