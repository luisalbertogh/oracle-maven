...
--changeset ${vr_owner_user}_CODE:PACKAGE_BODY_PKG_REPORTS_REFAC runOnChange:TRUE  failOnError:TRUE splitStatements:FALSE stripComments:FALSE


  CREATE OR REPLACE PACKAGE BODY "PKG_REPORTS_REFAC" 
AS
  -----------------------------------------------------------------------------
...
------------------------------------------------------------------------------
  ...
  IS
  BEGIN
    RETURN (F_CONVERT_RATE_CRP_REFAC(A_NUMBER,A_DATE,A_CRP))/1000;
  END F_C_RATE_CRPT_REFAC;
-----------------------------------------------------------------------------
...
------------------------------------------------------------------------------
  ...
  IS
  BEGIN
    RETURN F_CONVERT_RATE_CRP_REFAC(A_NUMBER,A_DATE,A_CRP)/1000000;
  END F_C_RATE_CRPM_REFAC;
----------------
-----------------------------------------------------------------------------
...
------------------------------------------------------------------------------
  ...
  IS
  BEGIN
    RETURN (F_CONVERT_RATE_VTD_REFAC(A_NUMBER,A_DATE))/1000;
  END F_C_RATE_VTDT_REFAC;
-----------------------------------------------------------------------------
-- Functionality: Get conversion rate for VTD in thousands
-- Used:
------------------------------------------------------------------------------
  ...
  IS
  BEGIN
    RETURN (F_CONVERT_RATE_VTD_REFAC(A_NUMBER,A_DATE,A_REGION))/1000;
  END F_C_RATE_VTDT_REFAC;
-----------------------------------------------------------------------------
...
------------------------------------------------------------------------------
  ...
  IS
  BEGIN
    RETURN (F_CONVERT_RATE_VTD_REFAC(A_NUMBER,A_DATE))/1000000;
  END F_C_RATE_VTDM_REFAC;
-----------------------------------------------------------------------------
-- Functionality: Get conversion rate for VTD in million
-- Used:
------------------------------------------------------------------------------
  ...
  IS
  BEGIN
    RETURN (F_CONVERT_RATE_VTD_REFAC(A_NUMBER,A_DATE,A_REGION))/1000000;
  END F_C_RATE_VTDM_REFAC;
-----------------------------------------------------------------------------
...
------------------------------------------------------------------------
  ...
  WHEN OTHERS THEN
    ...
    RAISE;
  END F_MIN_HADOOP_LASTDATE_REFAC;
-----------------------------------------------------------------------------
...
------------------------------------------------------------------------------
  ...
  WHEN OTHERS THEN
    ...
    RAISE;
  END F_GET_COMPARISON_MONTH_REFAC;
-----------------------------------------------------------------------------
...
------------------------------------------------------------------------------
  ...
  BEGIN
    ...
    LOOP
      ...
      ELSE
        SELECT PKG_REPORTS_REFAC.F_GET_FX_RATE_REFAC(age_comparison_index.notional_currency_id,age_comparison_index.fx_currency_id,age_comparison_index.region_id,age_comparison_index.asofdate)
        INTO p_FX_RATES
        FROM dual;
    ...
    COMMIT;
  END PR_GET_FXRATE_COMP_CASH_REFAC;
-----------------------------------------------------------------------------
...
------------------------------------------------------------------------------
  ...
  BEGIN
    ...
    ELSE
      v_reg := A_REGION;
    END IF;
    v_curr := F_GET_CURRENCY_REG_REFAC(A_REGIONSS);
    SELECT spot
    INTO v_spot
    FROM fx_rates
    WHERE asofdate =A_DATE
...
------------------------------------------------------------------------------
  ...
  IS
  BEGIN
    RETURN (F_CONVERT_RATE_REG_REFAC(A_NUMBER,A_DATE,A_REGIONSS,A_REGION))/1000;
  END F_C_RATE_REGT_REFAC;
-----------------------------------------------------------------------------
...
------------------------------------------------------------------------------
  ...
  IS
  BEGIN
    RETURN F_CONVERT_RATE_REG_REFAC(A_NUMBER,A_DATE,A_REGIONSS,A_REGION)/1000000;
  END F_C_RATE_REGM_REFAC;
...
A_CHRG_B31TO60 age_comparison_der.CHRG_B31TO60%TYPE;
  ...
  BEGIN
    ...
    COMMIT;
  END PR_GET_FXRATE_COMP_DER_REFAC;
  PROCEDURE PR_GET_FX_RATE_REFAC(
      p_source_system_id IN SOURCE_SYSTEM.SOURCE_SYSTEM_ID%TYPE, p_date IN NEW_AGE.ASOFDATE%TYPE)
  IS
    p_FX_RATES  NUMBER;
  ...
  BEGIN
    ...
    LOOP
      SELECT PKG_REPORTS_REFAC.F_GET_FX_RATE_REFAC(age_rates_index.notional_currency_id,age_rates_index.fx_currency_id,age_rates_index.region_id,age_rates_index.asofdate)
      INTO p_FX_RATES
      FROM dual;
...
------------------------------------------------------------
  ...
  BEGIN
    ...
    END IF;
    RETURN p_asofdate;
  END F_GET_COMPARISON_MONTH_R_REFAC;
----------------------------------------------------------------------------------------------------------------------------------------------------------------------
...
----------------------------------------------------------------------------------------------------------------------------------------------------------------------
  ...
  BEGIN
    ...
    WHEN C_DATES_TO_REFRESH%NOTFOUND;
      UPDATE_BH_RPL_EXPANDED_REFAC (C_ASOFDATE);
    END LOOP;
...
----------------------------------------------------------------------------------------------------------------------------------------------------------------------
  ...
  AS
  BEGIN
    REFRESH_BH_RPL_EXPANDED_REFAC(
      IDATE,
      f_get_comparison_month_r_REFAC (IDATE, -1),
      f_get_comparison_month_r_REFAC (IDATE, -2),
      f_get_comparison_month_r_REFAC (IDATE, -3),
      f_get_comparison_month_r_REFAC (IDATE, -4),
      f_get_comparison_month_r_REFAC (IDATE, -5),
      f_get_comparison_month_r_REFAC (IDATE, -6),
      f_get_comparison_month_r_REFAC (IDATE, -7),
      f_get_comparison_month_r_REFAC (IDATE, -8),
      f_get_comparison_month_r_REFAC (IDATE, -9),
      f_get_comparison_month_r_REFAC (IDATE, -10),
      f_get_comparison_month_r_REFAC (IDATE, -11));
    COMMIT;
  EXCEPTION
  WHEN OTHERS THEN
    ROLLBACK;
...
LAST_MODIFICATION_USER,
  ...
  WHEN OTHERS THEN
    ROLLBACK;
  END REFRESH_BH_RPL_QV_EXPANDED_RF;
