...
--changeset ${vr_owner_user}_CODE:PACKAGE_BODY_PKG_VRI_LIMIT_DAILY runOnChange:TRUE  failOnError:TRUE splitStatements:FALSE stripComments:FALSE


  CREATE OR REPLACE PACKAGE BODY "PKG_VRI_LIMIT_DAILY" AS
  ...
  LOOP
    ...
    --------------------- INIT VALIDATION CSV correct -----------------------------------------------------
     ...
     END IF;
      ...
      PKG_COMMON_UTILS.P_CHECK_FORMAT_FIELD(field_name, field_value, C_SIZE_HUNDRED, cont_error, validation_msg);
	  --*********************************
	  --New Validation LIMIT_METHOD housl be one of this (MMi,UW,RMH)
	  --GBSVR-35509: Accepting also limit method DGO
	  
	  IF(field_value <> 'MMI' AND field_value <> 'UW' AND field_value <> 'RMH' AND field_value <> 'DGO') THEN
     ...
     END;
      ...
      lower_limit_usage := trunc(((limit_exposure/limit_size)*99),2);
		  ...
		  IF limit_usage < lower_limit_usage or limit_usage > upper_limit_usage
		   ...
		   END IF;
		END IF;
	  --validation using RD_DESK_LIMIT_CONFIG
	--if date is not right, reject it.  
	...
	IF aux_date = 'N' THEN
     ...
     SELECT COUNT(1)
     INTO c_desk_limit
     FROM RD_DESK_LIMIT_CONFIG
     WHERE volcker_trading_desk = p_desk_id
	 AND limit_method = p_limit_method
	 AND asofdate = (select max (asofdate) from RD_DESK_LIMIT_CONFIG where asofdate<=TO_DATE(p_asofdate,'DD/MM/YYYY')) 
	 AND p_limit_method = 'MMI'
	 AND reporting_status = 'Y';
	END IF;
  
    IF (c_desk_limit > 0) THEN
      validation_code:= 1112;
      validation_msg := validation_msg || PKG_COMMON_UTILS.F_GET_VALIDATION_MSG(validation_code,'desk_id,limit_method',message_table);
      cont_warning    := cont_warning + 1;
      csv_true       := false;
    END IF;
	...
	OPEN c1;
      ...
      IF n_errors = 0 AND f_errors=0 AND f_errors_message=0 THEN   
        ...
        SELECT UPLOAD_ID,DESK_NAME,
            DESK_ID,
            TO_DATE(REPORTING_MONTH,'MM/YYYY'),
...
PROCEDURE INSERT_CSV(CSV_NAME IN VARCHAR, CSV_CONTENT IN VARCHAR)
IS
id_ref_data_table INTEGER;
BEGIN
  id_ref_data_table:=SEQ_REF_DATA_UI_UPLOAD.NEXTVAL;
  INSERT INTO REF_DATA_UI_UPLOAD (ID,FILE_TYPE_ID,STATUS_ID,FILENAME,UPLOADED_BY,UPLOADED_ON,CSV)
  VALUES(id_ref_data_table,2,3,CSV_NAME,'daily_user',current_timestamp,CSV_CONTENT);
  P_SPLIT_CSV(id_ref_data_table);
END INSERT_CSV;


END PKG_VRI_LIMIT_DAILY;
