...
--changeset ${vr_owner_user}_CODE:PACKAGE_PKG_REFERENCE_DATA runOnChange:TRUE  failOnError:TRUE splitStatements:FALSE


  CREATE OR REPLACE PACKAGE "PKG_REFERENCE_DATA" as
 ...
 TYPE TYPE_RESULTSET IS REF CURSOR;
  ...
  FUNCTION F_GET_COB_DATE (A_REGION_ID IN REGIONS.REGION_ID%TYPE,A_COB_DATE OUT REGIONS.COB_DATE%TYPE) RETURN NUMBER;
  FUNCTION F_GET_PREV_DATE (A_REGION_ID IN REGIONS.REGION_ID%TYPE,A_PREV_DATE OUT REGIONS.PREV_DATE%TYPE) RETURN NUMBER;
  FUNCTION F_GET_PREPREV_DATE (A_REGION_ID IN REGIONS.REGION_ID%TYPE,A_PREPREV_DATE OUT REGIONS.PREPREV_DATE%TYPE) RETURN NUMBER;

  FUNCTION F_GET_FX_RATES (A_region_id REGIONS.REGION_ID%TYPE,A_RATES_RESULTSET OUT TYPE_RESULTSET) RETURN NUMBER ;
  FUNCTION F_GET_LAST_FX_RATES (A_region_id REGIONS.REGION_ID%TYPE,A_cobdate FX_RATES.ASOFDATE%TYPE,A_RATES_RESULTSET OUT TYPE_RESULTSET) RETURN NUMBER ;
  FUNCTION F_GET_ASOFDATE_FX_RATES (A_source_system_id SOURCE_SYSTEM.SOURCE_SYSTEM_ID%TYPE,A_cobdate FX_RATES.ASOFDATE%TYPE,A_RATES_RESULTSET OUT TYPE_RESULTSET) RETURN NUMBER ;

  FUNCTION F_GET_FX_CURRENCIES (A_REGION_ID REGIONS.REGION_ID%TYPE,A_ASOFDATE AGE.ASOFDATE%TYPE,A_CURRENCIES_RESULTSET OUT TYPE_RESULTSET) RETURN NUMBER ;

  FUNCTION F_INSERT_FX_RATES (A_asofdate FX_RATES.ASOFDATE%TYPE,A_region_id REGIONS.REGION_ID%TYPE,A_CURRENCY_ID FX_RATES.CURRENCY_ID%TYPE,A_SPOT FX_RATES.SPOT%TYPE,A_fx_date FX_RATES.FX_DATE%TYPE) RETURN NUMBER ;
...
----------------------------------------------------------------------------------------------------------------------------------------------------------------------
  ...
  FUNCTION F_CALC_CHARGE_THRESHOLD(p_source_system_id SOURCE_SYSTEM.SOURCE_SYSTEM_ID%TYPE, p_asofdate AGE.ASOFDATE%TYPE, p_region_id FX_RATES.REGION_ID%TYPE) RETURN NUMBER;
  
  FUNCTION F_GET_REPORT_TRADING_DESK(p_metric_id  IN SOURCE_SYSTEM_METRIC.METRIC_ID%TYPE DEFAULT 'IA', p_asofdateIni IN AGE.ASOFDATE%TYPE, p_asofdateEnd IN AGE.ASOFDATE%TYPE) RETURN TYPE_RESULSET_UNIX;
