...
--changeset ${vr_owner_user}_CODE:PACKAGE_BODY_PKG_REFERENCE_DATA_REFAC runOnChange:TRUE  failOnError:TRUE splitStatements:FALSE stripComments:FALSE


  CREATE OR REPLACE PACKAGE BODY "PKG_REFERENCE_DATA_REFAC" 
AS
  FUNCTION F_GET_COB_DATE_REFAC(
  ...
  WHEN OTHERS THEN
    RETURN 1;
  END F_GET_COB_DATE_REFAC;
  FUNCTION F_GET_PREV_DATE_REFAC(
      A_REGION_ID IN REGIONS.REGION_ID%TYPE,
  ...
  WHEN OTHERS THEN
    RETURN 1;
  END F_GET_PREV_DATE_REFAC;
  FUNCTION F_GET_PREPREV_DATE_REFAC(
      A_REGION_ID IN REGIONS.REGION_ID%TYPE,
  ...
  WHEN OTHERS THEN
    RETURN 1;
  END F_GET_PREPREV_DATE_REFAC;
  FUNCTION F_GET_FX_RATES_REFAC(
  ...
  IS
    ...
    V_RESULT NUMBER;
  BEGIN
    v_result := pkg_reference_data_REFAC.f_get_prev_date_REFAC(a_region_id,v_prev_date);
  ...
  WHEN OTHERS THEN
    RETURN 1;
    RAISE;
  END F_GET_FX_RATES_REFAC;
  FUNCTION F_GET_LAST_FX_RATES_REFAC(
      A_region_id REGIONS.REGION_ID%TYPE,
  ...
  WHEN OTHERS THEN
    RETURN 1;
    RAISE;
  END F_GET_LAST_FX_RATES_REFAC;
  FUNCTION F_GET_ASOFDATE_FX_RATES_REFAC(
  ...
  WHEN OTHERS THEN
    RETURN 1;
    RAISE;
  END F_GET_ASOFDATE_FX_RATES_REFAC;
  FUNCTION F_GET_FX_CURRENCIES_REFAC(
      A_REGION_ID REGIONS.REGION_ID%TYPE,
  ...
  BEGIN
    IF A_ASOFDATE IS NULL THEN
      V_RESULT    := PKG_REFERENCE_DATA_REFAC.F_GET_COB_DATE_REFAC(A_REGION_ID,V_PREV_DATE);
      IF V_RESULT != 0 THEN
        RETURN V_RESULT;
...
WHEN OTHERS THEN
  RETURN 1;
  RAISE;
END F_GET_FX_CURRENCIES_REFAC;
  FUNCTION F_INSERT_FX_RATES_REFAC(
...
----------------------------------------------------------------------------------------------------------------------------------------------------------------------
  ...
  BEGIN
    ...
    COMMIT;
  END PR_SET_QV_LOAD_DATES_REFAC;
----------------------------------------------------------------------------------------------------------------------------------------------------------------------
...
AND metric_id = '''||p_metric||'''';
  ...
  WHEN OTHERS THEN
    RAISE;
  END F_GET_HADOOP_LOAD_DATE_REFAC;
----------------------------------------------------------------------------------------------------------------------------------------------------------------------
-- Functionality: Returns the next load date for the given region/source system
-- Used: IA Hadoop processes
----------------------------------------------------------------------------------------------------------------------------------------------------------------------
  ...
  WHEN OTHERS THEN
    RETURN 1;
    RAISE;
  END F_SET_HADOOP_LOAD_DATES_REFAC;
  FUNCTION F_CALC_CHARGE_THRESHOLD_REFAC(
      p_source_system_id SOURCE_SYSTEM.SOURCE_SYSTEM_ID%TYPE,
      p_asofdate NEW_AGE.ASOFDATE%TYPE,
  ...
  END;
    ...
    FUNCTION F_GET_REPORT_TRADING_DESK_RF(p_metric_id  IN SOURCE_SYSTEM_METRIC.METRIC_ID%TYPE DEFAULT 'IA', p_asofdateIni IN AGE.ASOFDATE%TYPE, p_asofdateEnd IN AGE.ASOFDATE%TYPE) 
RETURN TYPE_RESULSET_UNIX 
IS
    v_number NUMBER;
...
BEGIN
    ...
    END F_GET_REPORT_TRADING_DESK_RF;
